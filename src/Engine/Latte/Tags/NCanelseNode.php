...
/**
 * <div n:can="my-permission"> ... <div n:canelse>
 */
final class NCanelseNode extends StatementNode
{
    ...
    private static function processFragment(array $children): array
    {
        ...
        for ($i = 0; isset($children[$i]); $i++) {
            ...
            if ($child instanceof CanNode) {
                ...
                $nElse = $child;
                if ($currentNode === null) {
                    throw new CompileException('n:canelse must be immediately after n:can', $nElse->position);
                } elseif ($currentNode->else) {
                    throw new CompileException('Multiple "canelse" found.', $nElse->position);
                }

                if ($nElse->condition) {
                    $elseIfNode = new CanNode();
